...
.. math::
    ...
    F(q,alpha) = 3Delta ho V frac{(sin qr  - qr cos qr)}{(qr)^3}

for

.. math::

    r = left[ R_e^2 sin^2 alpha + R_p^2 cos^2 alpha ight]^{1/2}
...
radius and
$Delta ho$ is the scattering length density difference between
...
.. math::
    ...
    mathbf{e}_A = left( cos psi, sin psi sin gamma_1, sin psi cos gamma_1 ight)
...
.. math::
    ...
    mathbf{n}_0 = left( -sin psi, cos psi sin gamma_1, cos psi cos gamma_1 ight)
...
The angle $eta$ allows to choose the direction of the applied magnetic field, with $eta=90$ Degrees the field aligned along the horizontal axis, and for $eta=0$ Degrees the field directed along the beam direction.
...
parameters = [["sld", "1e-6/Ang^2", 40, [-inf, inf], "",
               "Ellipsoid scattering length density"],
              ["sld_solvent", "1e-6/Ang^2", 8, [-inf, inf], "",
