...
The model is based on spherical particles with a core multishell structure.
The magnetic scattering length density (SLD) is defined as $ho_{mathrm{mag}}=b_H M_S$, 
...
The magnetic SLD $ho_{mathrm{mag}}$ is hence weighted for each magnetisation component with the corresponding average of a Boltzmann orientation distribution 
...
.. math::
    langle M_{perp}angle=0;,
    langle M_{parallel}angle=F_m L(eta);,
    langle M_{perp}^2angle=F_m^2 L(eta)/eta;,
    langle M_{parallel}^2angle=F_m^2 (1-2L(eta)/eta)

with $F_m$ the magnetic scattering amplitude, the Langevin function $langle cos alphaangle= L(eta)=coth(eta)-1/eta$ and $M_{perp}$ and $M_parallel$ the two transversal and the in-field magnetisation components, respectively (for a detailed discussion see [#Muehlbauer2019]_). The transversal magnetisation components of core, shell and solvent are often considered coaligned, corresponding to a coupling parameter $delta =1$, and 0 if the transversal magnetisation of core and shell are uncorrelated (random orientational relation), i.e. the shell magnetisation component (perpendicular to the field) points in an arbitrary direction with respect to the core spin deflection. The scattering intensity reduces in this case to the sum over the individual squared form factors. The average of the crossed terms $F_{M,i}cdot F_{M,j}$ disappear as the transversal magnetisation components $F_{M,i}$ and $F_{M,j}$ of core and ,e.g., a shell can take all orientations with equal propability.

The fraction of "upward" neutrons before ('up_i') and after the sample ('up_f') must range between 0 to 1, with 0.5 denoting an unpolarised beam. Note that a fit may result in a negative magnetic SLD, and hence magnetisation, when the polarisation state is inverted, i.e. if you have analysed for a $I_{00}$ state wheras your data are $I_{11}$. The model allows to construct the 4 spin-resolved cross sections (non-spin-flip $I_{00}$, $I_{11}$ and spin-flip, here $I_{01}=I_{10}$), half-polarised SANS (SANSpol, incoming polarised beam $I_0$ and $I_1$, no analysis after sample 'up_frac_f'$=0.5$), and unpolarised beam ('up_i'$=$'up_f'$=0.5$).
...
The model was tested and gives correct results with the correct scaling and weighting in comparison with a simple sphere in 1 D and 2 D with transversal field and magnetic field parallel to the beam.

Further testing to demonstrate that the model reproduces known results required.
...
* **Last Modified by: Dirk Honecker **Date:** February 13, 2020
* **Last Reviewed by:**
